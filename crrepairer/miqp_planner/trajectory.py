...
class TrajPoint(object):
    def __init__(
        ...
        assert (
            isinstance(v, val.ValidTypes.NUMBERS) and v >= -1.0
        ), "<TrajPoint>: Provided velocity not valid. v= {}".format(v)
        assert isinstance(
            a, val.ValidTypes.NUMBERS
        ), "<TrajPoint>: Provided acceleration not valid. a= {}".format(a)
        assert isinstance(
            x, val.ValidTypes.NUMBERS
        ), "<TrajPoint>: Provided x coordinate not valid. x= {}".format(x)
        assert isinstance(
            y, val.ValidTypes.NUMBERS
        ), "<TrajPoint>: Provided y coordinate not valid. y= {}".format(y)
        assert isinstance(
            theta, val.ValidTypes.NUMBERS
    ...
    def __str__(self):
        return (
            "<TrajPoint>:  (t={}, position={}, orientation={}, v={}, a={}, j ={}, "
...
class Trajectory(object):
    def __init__(self, traj_points: list, coord_type: TrajectoryType) -> object:
        assert isinstance(
            coord_type, TrajectoryType
        ), "<Trajectory>: Provided trajectory type is not valid. type = {}".format(
            coord_type
        )
        assert isinstance(traj_points, list) and (
            isinstance(t, TrajPoint) for t in traj_points
        ), "<Trajectory>: Provided trajectory points invalid. points = {}".format(
            traj_points
        )
        assert self.__valid_trajectory(
            traj_points
        ), "<Trajectory>: Provided trajectory has invalid time information. points = {}".format(
            traj_points
        )
    ...
    def reset_time(self, t_0: val.ValidTypes.NUMBERS):
        ...
        """
        assert val.is_real_number(t_0), (
            "<Trajectory/reset_time>: provided t_0 is not valid! t_0 new = {} t_0 "
    ...
    def __valid_trajectory(self, traj_points: list):
        ...
        for i in range(1, len(traj_points)):
            ...
            if not np.isclose(dT, traj_points[i].t - traj_points[i - 1].t):
                LOGGER.warning(
                    "Expected dt = {} but received {} at i={}".format(
                        dT, traj_points[i].t - traj_points[i - 1].t, i
                    )
                )
                valid = False
                i = len(traj_points)
    ...
    def convert_to_cr_trajectory(
        self, wheelbase: float, wheelbase_ra: float, time_step_offset: int = 0
    ):
        """
        ...
        for idx, p in enumerate(self.states):
            state_values = {
                "position": p.position
                + wheelbase_ra
                * np.array([np.cos(p.orientation), np.sin(p.orientation)]),
                "velocity": float(p.v),  # x[0, 1] / np.cos(x[1, 1] - self.theta_ref),
                "acceleration": float(p.a),
                "jerk": float(p.j),
                "orientation": make_valid_orientation(float(p.orientation)),
                "slip_angle": float(p.kappa),
                "yaw_rate": float(p.kappa_dot),
                "time_step": int(round(p.t / self.dT)) + time_step_offset,
            }

            if idx == 0:
                state_values.update(
                    {
                        "velocity_z": 0.0,  # jerk_dot
                        "roll_rate": 0.0,  # kappa_dot_dot
                    }
                )
            else:
                state_values.update(
                    {
                        "velocity_z": self.u_lon[idx - 1],  # jerk_dot
                        "roll_rate": self.u_lat[idx - 1],  # kappa_dot_dot
                    }
                )
        ...
        # TODO adjust initial time step
        return cr_traj.Trajectory(time_step_offset, traj_cr)
    ...
    def convert_to_cr_ego_vehicle(
        ...
        ), "<Trajectory>: Provided width of vehicle invalid! width = {}".format(width)
        assert (
            isinstance(length, (int, float)) and length > 0
        ), "<Trajectory>: Provided width of vehicle " "invalid! width = {}".format(
            length
        )

        # get trajectory
        traj = self.convert_to_cr_trajectory(wheelbase, wheelbase_ra)
        ...
        ego = DynamicObstacle(
            ...
            initial_state=InitialState(
                position=traj.state_list[0].position,
                velocity=traj.state_list[0].velocity,
                orientation=traj.state_list[0].orientation,
                acceleration=traj.state_list[0].acceleration,
                yaw_rate=0.0,
                slip_angle=0.0,
            ),
        )
    ...
    def concatenate_trajectories(
        cls, pre: "Trajectory", post: "Trajectory"
    ) -> "Trajectory":
        """
        ...
        assert isinstance(
            pre, Trajectory
        ), "<Trajectory/concatenate>: pre trajectory is not valid! pre = {}".format(pre)
        assert isinstance(
            post, Trajectory
        ), "<Trajectory/concatenate>: post trajectory is not valid! post = {}".format(
            post
        )
        assert (
            pre.states[-1] == post.states[0]
        ), "<Trajectory/concatenate>: pre and post are not connected! pre = {} and post = {}".format(
            pre.states[-1], post.states[0]
        )
        assert pre.coord_type == post.coord_type, (
            "<Trajectory/concatenate>: pre and post are in different "
    ...
    def split_at_state(self, index: int) -> ("Trajectory", "Trajectory"):
        ...
        assert val.is_natural_number(
            index
        ), "<Trajectory/split_at_state>: Provided index is not valid! index = {}".format(
            index
        )
        assert val.is_in_interval(index, 0, self.N - 1), (
            "<Trajectory/split_at_state>: Provided index "
        ...
        if index < (self.N - 1):
            ...
            post = []
            LOGGER.warning("Empty trajectory since requested index is last state!")
    ...
    def split_at_time(self, t: float) -> ("Trajectory", "Trajectory"):
        ...
        assert val.is_real_number(t) and np.greater_equal(t, 0.0), (
            "<Trajectory/split_at_time>: Provided time "
            "is not valid! time = {}".format(t)
        )
        assert val.is_in_interval(t, self.t_0, self.t_h), (
            "<Trajectory/split_at_time>: Provided time is outside "
        ...
        if not np.equal(dist[index], 0.0):
            LOGGER.warning(
                "<Trajectory/split_at_time>: provided time is between two states!"
            )
    ...
    def split_at_position(self, pos: list) -> ("Trajectory", "Trajectory"):
        ...
        assert val.is_real_number_vector(pos, length=2), (
            "<Trajectory/split_at_position>: Provided position "
        ...
        threshhold = 130 / 3.6 / (1.0 / self.dT)
        if np.greater_equal(dist[index], threshhold):
            LOGGER.warning(
                "<Trajectory/split_at_position>: provided position seems to be too far away! pos = {}".format(
                    pos
                )
            )
    ...
    def shorten(self, horizon: float) -> "Trajectory":
        ...
        assert val.is_positive(
            horizon
        ), "<Trajectory/shorten>: Provided horizon is not valid! horizon = {}".format(
            horizon
        )
        assert np.greater_equal(self.t_h - self.t_0, horizon), (
            "<Trajectory/shorten>: Provided horizon is larger "
            "than current one! horizon {} <= {}".format(horizon, self.t_h - self.t_0)
        )
        ...
        if index == self.N - 1:
            LOGGER.warning(
                "<Trajectory/shorten>: Trajectory is shortened at last state; horizon remains the same!"
            )

        return self.split_at_state(index)[0]

    def resample(
        self, output_sampling_time, subsampling_factor=1
    ) -> (list, list, list, list):
        planning_steps = len(self.u_lon)
        LOGGER.info("DT IS = {}".format(self.dT))
        ...
        prediction_time = np.arange(
            planning_time[0], planning_time[-1], prediction_sampling_time
        )
        ...
        A = np.array(
            [
                [1, dt, dt**2 / 2.0, dt**3 / 6.0],
                [0, 1, dt, dt**2 / 2],
                [0, 0, 1, dt],
                [0, 0, 0, 1],
            ]
        )
        B = np.array([dt**4 / 24.0, dt**3 / 6.0, dt**2 / 2.0, dt])

        x_0 = np.array(
            [
                self.states[0].position[0],
                self.states[0].v,
                self.states[0].a,
                self.states[0].j,
            ]
        ...
        x_0 = np.array(
            [
                self.states[0].position[1],
                self.states[0].orientation,
                self.states[0].kappa,
                self.states[0].kappa_dot,
            ]
        ).transpose()
        ...
        for i in range(len(input_idxs)):
            ...
            A = np.array(
                [
                    [1, dt * v, dt**2 * 0.5 * (v**2), (dt**3 / 6.0) * (v**2)],
                    [0, 1, dt * v, (dt**2 / 2) * v],
                    [0, 0, 1, dt],
                    [0, 0, 0, 1],
                ]
            ...
            B = np.array(
                [(dt**4 / 24.0) * (v**2), (dt**3 / 6.0) * v, (dt**2 / 2.0), dt]
            )
    ...
    def sub_trajectory(self, t_0, N):
        ...
        return (
            Trajectory(self.states[index_low : (index_up + 1)], self.coord_type)
            if index_low < index_up - 1
            else []
        )
